...
func IntegrationTest() error {
	...
	var walker collectingWalker
	filepath.Walk("shakespeare", walker.Walk)

	var pathBuffer bytes.Buffer

	for _, pth := range walker.Paths {
		pathBuffer.WriteString(fmt.Sprintf("%s\n", pth))
	}

	cmd := exec.Command("pargs", "-pool", "2", "echo", "plays:")
	cmd.Stdin = &pathBuffer
...
var portBasename = fmt.Sprintf("pargs-%s", pargs.Version)

// repoNamespace identifies the Go namespace for this project.
var repoNamespace = "github.com/mcandre/pargs"
...
func Uninstall() error { return mageextras.Uninstall("pargs") }
